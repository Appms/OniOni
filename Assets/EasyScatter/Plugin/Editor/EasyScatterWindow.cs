...
public class EasyScatterWindow : EditorWindow {
	...
	static string date="4th December 2015";
	...
	void DrawLayerInspector(){
		...
		if (ESGuiTools.Button("Get",Color.white,50,size)){
			...
			if (tmpobj){
				...
				if (rootPivot.GetComponent<ScatterRoot>()){
					scatteredObjects = rootPivot.GetComponent<ScatterRoot>().scatteredObjects;
				}
				else{
	...
	void DrawBrushInspector(){
		...
		brush.size = EditorGUILayout.Slider(new GUIContent("Size",null,"Page Up / Page Down"),brush.size,0,100);
		brush.amount = EditorGUILayout.IntSlider("Amount",brush.amount,1,100);
		brush.flux = EditorGUILayout.Slider("Flux",brush.flux,0,50);
	...
	void DrawPrefabInspector(){
		...
		if (ESGuiTools.Button("Unselect",Color.white,145,18)){
		...
		for (int i=0;i<scatteredObjects.Count;i++){
			...
			GUI.enabled = !scatteredObjects[i].isWait2delete;
			bool tmpButtonState = ESGuiTools.ToogleButton(scatteredObjects[i].enable,scatteredObjects[i].prefab.name);

			if (Event.current.control){
				scatteredObjects[i].enable = tmpButtonState;
			}
			else{
				if (!scatteredObjects[i].enable && tmpButtonState ){
					scatteredObjects[i].enable = true;
					
					List<ScatteredObject> tmpobject = scatteredObjects.FindAll( delegate( ScatteredObject s) {
						return s.enable==true && s!=scatteredObjects[i];
					}
					);
			...
			// options
			if (!scatteredObjects[i].isWait2delete){

				 btnColor = Color.white;


				// Scale
				if (scatteredObjects[i].isScale) btnColor = Color.green;
				if (ESGuiTools.Button("",btnColor,size+4,size,false,GetIcon(0))){
					scatteredObjects[i].isScale = !scatteredObjects[i].isScale;
				}
				btnColor = Color.white;


				// X Rotate
				if (scatteredObjects[i].isRotationX) btnColor = Color.green;
				if (ESGuiTools.Button("",btnColor,size+4,size,false,GetIcon(1))){
					scatteredObjects[i].isRotationX = !scatteredObjects[i].isRotationX;
				}
				btnColor = Color.white;


				// Y Rotate
				if (scatteredObjects[i].isRotationY) btnColor = Color.green;
				if (ESGuiTools.Button("",btnColor,size+4,size,false,GetIcon(2))){
					scatteredObjects[i].isRotationY = !scatteredObjects[i].isRotationY;
				}
				btnColor = Color.white;

				// Z Rotate
				if (scatteredObjects[i].isRotationZ) btnColor = Color.green;
				if (ESGuiTools.Button("",btnColor,size+4,size,false,GetIcon(3))){
					scatteredObjects[i].isRotationZ = !scatteredObjects[i].isRotationZ;
				}
				btnColor = Color.white;

				// Options
				if (scatteredObjects[i].showOption) btnColor = Color.green;
				if (ESGuiTools.Button("",btnColor,size+4,size,false,GetIcon(4))){
					scatteredObjects[i].showOption = !scatteredObjects[i].showOption;
				}
			}
			
			// Delete
			if (ESGuiTools.Button("",Color.white,size+4,size,false,GetIcon(5))){
				scatteredObjects[i].isWait2delete = !scatteredObjects[i].isWait2delete;
			}
			...
			#region options
			if (!isDelete && scatteredObjects[i].showOption){
				
				ESGuiTools.BeginGroup();
				
				scatteredObjects[i].offset = EditorGUILayout.FloatField("Vertical offset",scatteredObjects[i].offset);
				
				scatteredObjects[i].isScaleOption = ESGuiTools.SimpleFoldOut("Scale options",scatteredObjects[i].isScaleOption);
				if (scatteredObjects[i].isScaleOption){
					EditorGUI.indentLevel++;
					EditorGUILayout.BeginHorizontal();
					scatteredObjects[i].uniformScale = EditorGUILayout.ToggleLeft("Uniform",scatteredObjects[i].uniformScale,GUILayout.Width(80) );
					scatteredObjects[i].isScaleParticle = EditorGUILayout.ToggleLeft("Particle",scatteredObjects[i].isScaleParticle,GUILayout.Width(80));
					scatteredObjects[i].isScaleLight = EditorGUILayout.ToggleLeft("Light",scatteredObjects[i].isScaleLight,GUILayout.Width(100));
					EditorGUILayout.EndHorizontal();
					if (scatteredObjects[i].uniformScale){
						EditorGUILayout.BeginHorizontal();
						scatteredObjects[i].xMinScale = EditorGUILayout.FloatField("",scatteredObjects[i].xMinScale, GUILayout.Width(50));
						EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].xMinScale, ref scatteredObjects[i].xmaxScale,0,10);
						scatteredObjects[i].xmaxScale = EditorGUILayout.FloatField("",scatteredObjects[i].xmaxScale, GUILayout.Width(50));
						EditorGUILayout.EndHorizontal();
						...
						scatteredObjects[i].yMinScale = EditorGUILayout.FloatField("",scatteredObjects[i].yMinScale, GUILayout.Width(50));
						EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].yMinScale, ref scatteredObjects[i].yMaxScale,0,10);
						scatteredObjects[i].yMaxScale = EditorGUILayout.FloatField("",scatteredObjects[i].yMaxScale, GUILayout.Width(50));
						EditorGUILayout.EndHorizontal();
						
						EditorGUILayout.BeginHorizontal();
						EditorGUILayout.LabelField("Z",GUILayout.Width(30));
						scatteredObjects[i].zMinScale = EditorGUILayout.FloatField("",scatteredObjects[i].zMinScale, GUILayout.Width(50));
						EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].zMinScale, ref scatteredObjects[i].zMaxScale,0,10);
						scatteredObjects[i].zMaxScale = EditorGUILayout.FloatField("",scatteredObjects[i].zMaxScale, GUILayout.Width(50));
						EditorGUILayout.EndHorizontal();
					}
					EditorGUI.indentLevel--;
				}
				
				scatteredObjects[i].isRotationOption = ESGuiTools.SimpleFoldOut("Rotation options",scatteredObjects[i].isRotationOption);
				if (scatteredObjects[i].isRotationOption){
					EditorGUI.indentLevel++;
					
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("X",GUILayout.Width(30));
					scatteredObjects[i].xMinRot = EditorGUILayout.FloatField("",scatteredObjects[i].xMinRot, GUILayout.Width(50));
					EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].xMinRot, ref scatteredObjects[i].xMaxRot,-180,180);
					scatteredObjects[i].xMaxRot = EditorGUILayout.FloatField("",scatteredObjects[i].xMaxRot, GUILayout.Width(50));
					EditorGUILayout.EndHorizontal();
					
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("Y",GUILayout.Width(30));
					scatteredObjects[i].yMinRot = EditorGUILayout.FloatField("",scatteredObjects[i].yMinRot, GUILayout.Width(50));
					EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].yMinRot, ref scatteredObjects[i].yMaxRot,-180,180);
					scatteredObjects[i].yMaxRot = EditorGUILayout.FloatField("",scatteredObjects[i].yMaxRot, GUILayout.Width(50));
					EditorGUILayout.EndHorizontal();
					
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("X",GUILayout.Width(30));
					scatteredObjects[i].zMinRot = EditorGUILayout.FloatField("",scatteredObjects[i].zMinRot, GUILayout.Width(50));
					EditorGUILayout.MinMaxSlider(ref scatteredObjects[i].zMinRot, ref scatteredObjects[i].zMaxRot,-180,180);
					scatteredObjects[i].zMaxRot = EditorGUILayout.FloatField("",scatteredObjects[i].zMaxRot, GUILayout.Width(50));
					EditorGUILayout.EndHorizontal();
					EditorGUI.indentLevel--;
				}
	...
	void DrawSelectionTools(){
		...
		if (GUILayout.Button("X rotation")){
			SelectionRotate( dynamicSelections.ToArray(),scatteredModel,0);
		}
		if (GUILayout.Button("Y rotation")){
			SelectionRotate( dynamicSelections.ToArray(),scatteredModel,1);
		}
		if (GUILayout.Button("Z rotation")){
			SelectionRotate( dynamicSelections.ToArray(),scatteredModel,2);
		}
		EditorGUILayout.EndHorizontal();
		ESGuiTools.EndGroup();
		ESGuiTools.EndGroup();
	...
	void DrawInfosInspector(){
		...
		EditorGUILayout.Space();
		EditorGUILayout.LabelField("Hold down shift + previous opt\t:");
	...
	void DrawPreferenceInspector(){
		ESGuiTools.SimpleTitle("brush preference");
		...
		if (objs.Count>0){
			...
			for (int o=0;o<brush.amount;o++){
				...
				if (Physics.Raycast(pos +  normal.normalized,-normal,out hit,Mathf.Infinity,brush.pickableLayer )){
					...
					GameObject obj =  (GameObject)Instantiate(objs[rndObj].prefab, pos, Quaternion.identity);
					Undo.RegisterCreatedObjectUndo (obj, "Easy Scatter : Painted object");

					// Apply random
					objs[rndObj].ApplyRS( obj.transform);
	...
	void SelectionRotate( Transform[] transforms,ScatteredObject model, int axis){
