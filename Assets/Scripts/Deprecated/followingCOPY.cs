...
public class followingCOPY : MonoBehaviour
{
    GameObject leader;
    ...
    void FixedUpdate()
    {
        leaderVel = leader.GetComponent<LeaderMovement>().velocity;
    ...
    private Vector3 separate()
    {
        ...
        for (int i = 0; i < boids.Length; i++)
        {
            ...
            if (/*boid != this*/ separation.magnitude > 0 && separation.magnitude <= SEPARATION_RADIUS)
            {
                ...
                added_force += (separation * Mathf.Pow(Vector3.Distance(transform.position, behind), 1 / 10f) / squaredSeparation); // function
                neighborCount++;
            }
        }
    ...
    private Vector3 evadeCollider(Collider other)
    {
        ...
        if (myNormal.magnitude > 0)
        {
            ...
            if (Vector3.Distance(transform.position + orto, leader.transform.position) >= Vector3.Distance(transform.position - orto, leader.transform.position)) avoidance_force -= orto;
            else avoidance_force += orto * Mathf.Abs(Vector3.Dot(orto, behind - transform.position));
    ...
    void OnDrawGizmos()
    {
       ...
       // Gizmos.DrawSphere(behind, 2);
        ...
        Gizmos.color = Color.red;
        Gizmos.DrawLine(transform.position, transform.position + collisionAvoidance.normalized * 5f);
